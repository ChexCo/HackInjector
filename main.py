...
# If you created a fan version, please credit us.
...
def bsod():
    text = input('What should be the error code?: ')
    username = input("What is the username of the user you want to have a glitch?: ")  
    a = getIP(username)
    if a != None:
        print(f'Got the IP: {a}')
        sleep(0.5)
        print('Trying to BSOD...')
        b = randint(1, 5)
        if b != 2 or 4:
            print(f'{Fore.LIGHTRED_EX}No Access!{Fore.RESET}')
            sleep(2)
        else:
            print('Got Access!')
            sleep(1)
            print('BSODding...')
            sleep(0.5)
            print(f"Fetching {username}'s screen...")
            sleep(1)
            print(f'Done! Now {user} has the BSOD with the error code {text}!')
            sleep(2)
def textCheck():
    ...
    action = input("What to do? (log/write/delete): ")
...
What do you choose? Note: used like 1), 2) etc.: """
    ...
    if options != None:
        if options.startswith('1)'):
            ...
            textCheck()
        elif options.startswith("16)"):
            print(f"{__name__} {__state__} v{__version__}")
        elif options.startswith("17)"):
            exit()
    else:
